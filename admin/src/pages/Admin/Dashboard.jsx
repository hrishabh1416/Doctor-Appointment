    ...
    <div className='m-5'>
      <div className='flex flex-wrap gap-3'>
        <div className='flex items-center gap-2 bg-white p-4 min-w-32 rounded border-2 border-gray-100 cursor-pointer hover:scale-105 transition-all'>
          <img src={assets.doctor_icon} className='w-14'></img>
          <div>
        ...
        <div className='pt-4 border border-t-0'>
          {
            dashData.latestappointments.map((item,index)=> (
                ...
                <img src={item.docData.image} className='rounded-full w-10'></img>
                <div className='flex-1 text-sm'>
                <p className='text-gray-800 font-medium'>{item.docData.name}</p>
                <p className='text-gray-600'>{item.slotDate}</p>
                </div>
                {item.cancelled?<p className='text-red-500'>Cancelled</p>:<img src={assets.cancel_icon} className='w-10 cursor-pointer' onClick={()=>cancelappointment(item._id)}></img>}
                </div>
            ))
          }
        </div>
