...
const DoctorAppointments = () => {
  const{dToken,appointments,getAppointments,completeappointment,canceleappointment}=useContext(DoctorContext)
          ...
          appointments.map((item,index)=>{
            return (
              ...
              <div className='flex items-center gap-2'>
               <img src={item.userData.image} className='w-8 rounded-full'></img> <p>{item.userData.name}</p>
              </div>
              <div>
                <p className='text-sm inline border border-black px-2 rounded-full'>{item.payment?'Online':'Cash'}</p>
              </div>
